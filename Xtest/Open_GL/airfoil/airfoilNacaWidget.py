...
class Naca5Tab(QtGui.QWidget):
    ...
    def fireButtonCreate(self):
        try : 
            cl        = float(self.text2Cl.text())
            posCamber = float(self.combosCamberPos.currentText() )
            thick     = float(self.text4Thickness.text())
            reflex    = self.combosCamberReflex.currentText() == "1"
            
            # reflex not possible at posCamber 1
            if posCamber == 1 and reflex:
                reflex = False           
            elif thick == 0 : 
                self.text4Thickness.selectAll()
                return            
            
            self.text1Name.setText('NACA ' + self.text2Cl.text() + self.combosCamberPos.currentText() 
                                   + ("1" if reflex else "0") + self.text4Thickness.text())
            
        except ValueError:
            print ("Exception fireButtonCreate in profileWidget (NACA Creator)" ,  thick)
            return
        
        self.ogl_widget.createCambered_Naca5(cl, posCamber, reflex, thick, self.countSpinBox.value())        
        self.ogl_widget.profile.setName(self.text1Name.text())