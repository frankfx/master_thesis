...
def naca5(naca1, naca23, naca45, n, finite_TE=False, half_cosine_spacing=False):
    
    # optimum lift coefficient at the ideal angle-of-attack --> (multiplied by 0.15)
    olc = naca1 * 1.5 / 10.0
    # point of max camber located at xx% chord --> (multiplied by 5) and reflex camber value (0=simple,1=reflex) 
    p = 0.5*naca23/100.0
    # max thickness (as a percentage of the chord)
    t = naca45/100.0
    ...
    if p == 0:
        ...
        zc = map(lambda xx : (olc/0.3) * xx, yc1 + yc2)

        dyc1_dx = map(lambda xx : olc/0.3*(1.0/6.0)*k1*( 3*math.pow(xx,2)-6*m*xx+math.pow(m,2)*(3-m) ), xc1);
        dyc2_dx = [olc/0.3*(1.0/6.0)*k1*math.pow(m,3)]*len(xc2);
